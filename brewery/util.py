...
def getIBU(recipe):
    ...
    for hop in recipe.hop_usages.all():
        if hop.use == "Dry Hop":
            hop.ibu_component = 0
            hop._ignore_hop_update = True
            hop.save()
        else:
            oldVal = hop.ibu_component
            ...
            if hop.form == "Pellet":
                ...
                hop.ibu_component = ibu
            elif hop.form == "Plug":
                ibu = ibu + ibu * DEFAULT_PLUG_ADJUST
                hop.ibu_component = ibu
            ibu_total += ibu
            hop._ignore_hop_update = True
            print("Saving the Hop ID:" + str(hop.id) + ", Old:" + str(oldVal) + " New:" + str(hop.ibu_component))
            hop.save()
    print("Done Saving the hops")
    return ibu_total
